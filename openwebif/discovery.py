"""Discovers enigma2s on the network using mDNS/zeroconf."""
import logging
import socket
...
class Enigma2Listener():
    """Zeroconf Enigma2 Services collection."""
    ...
    @property
    def count(self):
        """Number of discovered Enigma2 services."""
...
def start_discovery(add_callback=None, remove_callback=None):
    """
    Start discovering enigma2s on the network.

    This method will start discovering enigma2s on a separate thread. When
    a enigma2 is discovered, the callback will be called with the
    discovered enigma2's zeroconf name. This is the dictionary key to find
    the enigma2 metadata in listener.services.

    This method returns the Enigma2Listener object and the zeroconf
    ServiceBrowser object. The Enigma2Listener object will
    contain information for the discovered
    enigma2s. To stop discovery, call the stop_discovery method with the
    ServiceBrowser object.
    """
    listener = Enigma2Listener(add_callback, remove_callback)
    service_browser = False
    try:
        service_browser = zeroconf.ServiceBrowser(zeroconf.Zeroconf(),
                                                  "_e2stream._tcp.local.",
                                                  listener)
...
def stop_discovery(browser):
    """Stop the enigma2 discovery thread."""
    browser.zc.close()
...
def discover_enigma2s(max_devices=None, timeout=DISCOVER_TIMEOUT):
    ...
    try:
        # pylint: disable=unused-argument
        def callback(name):
            """Called when zeroconf has discovered a new enigma2."""
...
def get_info_from_service(service, zconf):
    ...
    service_info = None
    try:
        service_info = zconf.get_service_info('_e2stream._tcp.local.',
                                              service)
        if service_info:
            _LOGGER.debug(
